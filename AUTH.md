...
### 3. Гостевой режим (legacy)
- **Endpoint**: `POST /auth/guest`
- **Поля**: username
- **Особенности**: Без пароля, для быстрого доступа
...
### LoginForm
- Поля: username, password
- Переключатель гостевого режима
...
### Гостевой режим
1. Пользователь выбирает гостевой режим
2. Заполняет только username
3. Отправка POST /auth/guest
4. Создание временного пользователя
5. Возврат JWT токена
...
### Гостевой вход
```javascript
const response = await apiClient.authenticate('guestuser');
```
